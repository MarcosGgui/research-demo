...
//    }
    ...
    public List<Map> searchByScript(String esIndex, String searchFieldName, String scriptExpression)
        throws IOException {
        List<Map> resultList = new ArrayList<>();
        SearchRequest request = new SearchRequest(esIndex);
        Script script = new Script(scriptExpression);
        SearchSourceBuilder builder = new SearchSourceBuilder();
        builder.scriptField(searchFieldName, script);
        request.source(builder);
        SearchResponse response = client.search(request, RequestOptions.DEFAULT);
        SearchHits searchHits = response.getHits();
        SearchHit[] hits = searchHits.getHits();
        for (SearchHit hit : hits) {
            DocumentField hitMap = hit.getFields().get(searchFieldName);
            Map<String, Object> field = new HashMap<>();
            field.put(hitMap.getName(), hitMap.getValue());
